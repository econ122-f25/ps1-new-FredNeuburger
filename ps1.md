...
mtcars %>% filter(cyl == 4 | 6 & mpg > 20) %>% arrange(mpg, desc(cyl))
```
...
## There’s a consistent inverse relationship between horsepower and cylinders versus mpg. The cars at the top of the output have the highest number of cylinders in the subset and the lowest mpg.
...
``` r
# Your code here
mtcars_processed %>% mutate(hp_per_wt = hp / wt, qsec_category = case_when(
  qsec < 17 ~ "Fast",
...
## hp_per_wt helps normalize the cars’ strength relative to their weight. This makes the cars’ performance more comparable. A qsec_category simplifies the cars’ quarter mile performance for easier readability on the table.
...
``` r
# Your code here
mtcars_processed %>% group_by(cyl, am) %>% summarize(average_mpg = mean(mpg), median_hp = median(hp), number_of_cars = n())
```

    ## # A tibble: 6 × 5
    ## # Groups:   cyl [3]
    ##     cyl    am average_mpg median_hp number_of_cars
    ##   <dbl> <dbl>       <dbl>     <dbl>          <int>
    ## 1     4     0        22.9      95                3
    ## 2     4     1        28.1      78.5              8
    ## 3     6     0        19.1     116.               4
...
## A higher median hp is correlated with a higher number of cylinders and lower average mpg. In subsets of cars with the same number of cylinders, manual transmission cars (am: 1) have higher average mpg.
...
product_sales_long <- product_sales_wide %>%
    pivot_longer(cols = Year_2020:Year_2022, names_to = "Year", values_to = "Sales") %>% mutate(Year = as.numeric(str_remove(Year, "Year_")))
product_sales_long
```
